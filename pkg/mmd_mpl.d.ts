...
export class Vector3 {
  ...
  multiply_by_scalar(scalar: number): Vector3;
  x: number;
  y: number;
  z: number;
}
export class WasmMPLCompiler {
  ...
  reverse_compile(source: string, data: Uint8Array): string;
...
export interface InitOutput {
  ...
  readonly vector3_multiply_by_scalar: (a: number, b: number) => number;
  ...
  readonly wasmmplcompiler_reverse_compile: (a: number, b: number, c: number, d: number, e: number) => [number, number, number, number];
